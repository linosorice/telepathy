  ...
  spawnRoom () {
    ...
    for (var i = 0; i < ROOM_COLS; i++) {
      for (var j = 0; j < ROOM_ROWS; j++) {
        if (i === 0 && j === (ROOM_ROWS - 1) / 2) {  // north door
          if (map[x][y].u) {
        ...
        if (i === (ROOM_COLS - 1) / 2 && j === ROOM_ROWS - 1) {  // east door
          if (map[x][y].r) {
            this.game.add.existing(
              new Ground({
                game: this.game,
        ...
        if (i === (ROOM_COLS - 3) / 2 && j === ROOM_ROWS - 1) {  // east wall above door
          if (map[x][y].r) {
            this.game.add.existing(
              new Wall({
                game: this.game,
        ...
        if (i === ROOM_COLS - 1 && j === (ROOM_ROWS - 1) / 2) {  // south door
          if (map[x][y].d) {
            this.game.add.existing(
              new Ground({
                game: this.game,
        ...
        if (i === (ROOM_COLS - 1) / 2 && j === 0) {  // west door
          if (map[x][y].l) {
            this.game.add.existing(
              new Ground({
                game: this.game,
        ...
        if (i === (ROOM_COLS - 3) / 2 && j === 0) {  // west wall above door
          if (map[x][y].l) {
            this.game.add.existing(
              new Wall({
                game: this.game,
        ...
        this.game.add.existing(
          new Ground({
            ...
            x: TILE_SIZE_SPACED * (j - (ROOM_ROWS - 1) / 2) + this.game.world.centerX +
              ROOM_SIZE * (y - (BATTLEFIELD_ROWS - 1) / 2),
            north: i === 1 && (j !== (ROOM_COLS - 1) / 2 || map[x][y].u === false)
          })
        )
      }
    }
